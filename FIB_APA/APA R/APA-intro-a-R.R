####################################################################
# APA introducció a R (Grau FIB)
# Lluís A. Belanche

# Introduction to R (and a bit of statistics)
...
-1/0; 0/0; 1/0; sqrt(-2+0i) # use ';' to evaluate distinct expressions
...
## Irregular/regular sequences

a <- c(1,3,2,4,5)  # function concatenate 'c()'

# the () force to print the evaluation result
...
x[2]; x[-2]; x[c(1,3)]; x[rep(1:3,2)]   # index of positive/negative integers
...
gender <- rep(c("F","M"),c(3,2)); gender; mode(gender) # generate a character vector
...
X%*%X                     # matrix multiplication operator '%*%'
...
solve(X) # solve linear system of the form X = b
...
attach(mystuff); pro; pro[1:3] # component names can now be used directly 

pro/10; mystuff$pro            # changes in attached objects do not modify
                          # the source object

detach(mystuff)                # detach dataframe


mystuff[order(-mystuff$pro),]    # Rows from 'mystuff' displayed in decreasing order with respect to 'pro'
...
Z <- data.frame(x = c(1, 2, 3), y = c(0, 10, NA)); Z
...
z[is.element(z,c(-99,-999,"."))] <- NA; z  # convert '-99' and  '-999'
                                           # and '.' into NAs
z <- as.numeric(z); z                      # convert 'z' into numeric
...
mylist <- list(Day=c("Mo","Tu"),   # a list of R objects
...
for (i in 1:10) if ((g<-rnorm(1))>0.25) { cat(paste(i, round(g,4),"\n")) } else { cat("no!\n") }
...
my.sqr <- function(x) x^2 ; my.sqr(1:5)
...
# R can provide very quick summary information
 ...
 par(mfrow=c(3,1))
 for (i in 1:3) { plot(density(trees[,i]), xlab="Value", main=names(trees)[i]) }
...
with(trees, plot(Girth,Volume,main = "Black cherry trees growth", pch = 17))  # elegant way of accessing variables in a dataframe
...
# (handle this with care, if may mask errors)
...
# and a 3d scatterplot (that can be rotated)
...
#library(package) and require(package) both load the package with name package.

 
                       # in the local library file
