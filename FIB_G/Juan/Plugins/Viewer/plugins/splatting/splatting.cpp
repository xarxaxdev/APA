...
void Splatting::onPluginLoad()
{
    ...
    // FS
    QString fs_src = "#version 330 core\nuniform sampler2D sampler0; uniform sampler2D sampler1; uniform sampler2D sampler2; in vec2 vtexCoord; out vec4 fragColor; void main() { fragColor =  mix(texture(sampler1, vtexCoord), texture(sampler2, vtexCoord), texture(sampler0, vtexCoord).z); }";
