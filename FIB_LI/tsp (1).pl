...
%poscost-P-C en al posicio p hi ha d'haver el cost C => 
...
paritat(N1,N2):- 0 is mod(N1,2), 0 is mod(N2,2),!.
paritat(N1,N2):- 1 is mod(N1,2), 1 is mod(N2,2),!.
...
%if visited-1-0 -> visited-10-1
%if visited-10-1 -> visited7-2 v visited-16-2 v visited-ETC
%writeClause([\+visited-I-P1,\+cost-I]),

%optimitzar una miqueta and so on

gotoadjacent:- position2(P), node(I), findall(visited-N-P, nadjacents(I,N), Lits), P1 is P-1,writeClause([\+visited-I-P1|Lits]), fail.
gotoadjacent.

notcosting:- position2(P), node(I), nadjacents(I,N), paritat(I,N), P1 is P-1, writeClause([\+visited-I-P1,\+visited-N-P,\+cost-P]), fail.
notcosting.

costing:- position2(P), node(I), nadjacents(I,N), not(paritat(I,N)), P1 is P-1, writeClause([\+visited-I-P1,\+visited-N-P,cost-P]), fail.
costing.

costMax:- maxCost(C), findall(cost-P, position2(P),Lits), atMost(C,Lits), fail.
costMax.

eachNExactlyOneP:- node(N), N > 1, findall(visited-N-P, position2(P), Lits), exactly(1,Lits), fail.
eachNExactlyOneP.



%% Display solution
displaySol(M):- write(M), nl, position(P), member(visited-Node-P,M), write(Node), write(' '), fail.
displaySol(_):- nl.
