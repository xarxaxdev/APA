...
writeClauses:-
  eachSexactlyOneG,%cada estudiant té un sol grup
  sizeGroup,  %mirar maxim i minim de grup
  noSincompat,%no tenim dos estudiants incompatibles
  true.

eachSexactlyOneG:- student(S),
 findall(sg-S-G,group(G),Lits),
 exactly(1,Lits),
 fail.
eachSexactlyOneG.

sizeGroup:-
  group(G), minGroup(Min), maxGroup(Max),
  findall(sg-S-G,student(S),Lits),
  atLeast(Min,Lits),  atMost(Max,Lits),
  fail.
sizeGroup.
